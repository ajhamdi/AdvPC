...
parser.add_argument('--phase', type=str, default='attack', choices=['attack', 'evaluate','train','all'], help='perform attack or evaluate performed attack or both')
...
parser.add_argument('--dump_dir', default='features', help='dump folder path [perturbation]')

# features exps 
parser.add_argument('--features_type', type=str, default='non', choices=[
                    'non', 'robust','advrobust'], help='perform attack or evaluate performed attack or both')
parser.add_argument('--features_dir', default='features',
                    help='dump folder for current features')
parser.add_argument('--features_layer', type=str, default='0', help='pick ')
parser.add_argument('--features_id', type=str, default='0000', help='pick ')
parser.add_argument('--init', type=int, default=0,
                    help='the type of intiilizaitons for the inversion 0: random 1: original')

parser.add_argument('--std_dev', type=float, default=0.2,
                    help='the std deviation of the initialization of the shape with random initiailization')
parser.add_argument('--sigma', type=float, default=0,help='reggularize the featuers inversion with adv loss ')
# parser.add_argument('--features_size', type=int, default=1024,
#                     help='the featueres loaded ( to be inverted ) size')
...
# setup["batch_size"] = FLAGS.batch_size
# setup["num_point"] = FLAGS.num_point
GPU_INDEX = FLAGS.gpu
...
DUMP_DIR = os.path.join(DUMP_DIR, FLAGS.network)
FLAGS.dump_dir = DUMP_DIR
DATA_DIR = FLAGS.data_dir

check_folder(DUMP_DIR)
# TARGET=FLAGS.target
# setup["lr_attack"]=FLAGS.lr_attack
# setup["initial_alpha"] = FLAGS.initial_alpha
# setup["b_infty"] = FLAGS.b_infty
# setup["b_two"] = FLAGS.b_two
# setup["u_infty"] = FLAGS.u_infty
# setup["u_two"] = FLAGS.u_two
# setup["s_infty"] = FLAGS.s_infty
# setup["gamma"] = FLAGS.gamma

# setup["beta_two"] = FLAGS.beta_two
# setup["beta_infty"] = FLAGS.beta_infty
# setup["beta_cham"] = FLAGS.beta_cham
# setup["beta_emd"] = FLAGS.beta_emd

#WEIGHT=FLAGS.weight


# models["attacked_data_all"] = list(np.random.normal(0, FLAGS.std_dev, np.array(models["attacked_data_all"]).shape))


# setup["initial_weight"]=FLAGS.initial_weight
# setup["upper_bound_weight"]=FLAGS.upper_bound_weight
# #ABORT_EARLY=False
# setup["step"]=FLAGS.step
# setup["num_iter"]=FLAGS.num_iter
# setup["dyn_freq"] = FLAGS.dyn_freq
...
def attack(setup,models,targets_list,victims_list):
    ...
    with tf.Graph().as_default():
        ...
        ae = PointNetAutoEncoderWithClassifier(conf.experiment_name, conf)
            ...
            # pointclouds_pl, labels_pl = MODEL.placeholder_inputs(setup["batch_size"], setup["num_point"])
        ae.models = models
            ...
            for victim in victims_list:
                ...
                for j in range(NB_PER_VICTIM//setup["batch_size"]):
                    ...
                    norms, img = attack_one_batch(
                        sess, ops, c_attacked_data, setup)
                    np.save(os.path.join('.', setup["dump_dir"], '{}_{}_{}_{}_{}.npy' .format(
                        victim, setup["target"], j, setup["features_type"], setup["features_layer"])), img)
...
#########################################################################################
            ...
            for e, (c_loss, two, pred, ii, linfty, cham, emd) in enumerate(zip(features_loss, dist_two, early_pred_val, input_val, dist_infty, dist_cham, dist_emd)):
                ...
                # ae_cond = (d_pred != setup["victim"] or (setup["gamma"] < 0.001) or bool(setup["unnecessary"]))
                if dist < bestdist[e] :
        ...
        # for e in range(setup["batch_size"]):
...
def initialize(setup,models):
        ...
        models["attacked_data_all"] = joblib.load(
            os.path.join(setup["data_dir"], 'attacked_data.z'))
        ...
        setup["features_dir"] = os.path.join(
        setup["features_dir"], setup["features_id"], setup["network"])
        setup["batch"] = 0
        setup["features_size"] = {"0": (setup["batch_size"], 40), "1": (setup["batch_size"], 256), "2": (
            setup["batch_size"], 1024), "3": (setup["batch_size"], setup["num_point"], 1024),
            "4": (setup["batch_size"], 64, 64), "5": (setup["batch_size"], setup["num_point"], 64),
                "6": (setup["batch_size"], setup["num_point"], 3)}[setup["features_layer"]]
        ...
        models["PN_PATH_adv_ROBUST"] = os.path.join(
            BASE_DIR, "adv_robust_training", "PN", "model.ckpt")
        models["PN1_PATH_adv_ROBUST"] = os.path.join(
            BASE_DIR, "adv_robust_training", "PN1", "model.ckpt")
        models["PN2_PATH_adv_ROBUST"] = os.path.join(
            BASE_DIR, "adv_robust_training", "PN2", "model.ckpt")
        models["GCN_PATH_adv_ROBUST"] = os.path.join(
            BASE_DIR, "adv_robust_training", "GCN", "model.ckpt")
        ...
        models["test_path_adv_robust"] = copy.deepcopy(
            models[setup["network"]+"_PATH_adv_ROBUST"])

        if setup["features_type"] == "robust":
            models["test_path"] = models["test_path_robust"]
        elif setup["features_type"] == "advrobust":
            models["test_path"] = models["test_path_adv_robust"]
...
if __name__=='__main__':
    ...
    elif setup["phase"] == "train":
        # results = load_results(setup["load_file"])
        ev_results = train(setup, models, targets_list, victims_list)
